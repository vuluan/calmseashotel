...
class Room_services extends MX_Controller {
	...
	public function admincp_save(){
		...
		if($_POST){
			...
			if($this->model->saveManagement($fileName)){
				...
				if($_FILES){
					if($_FILES){
						$upload_path = BASEFOLDER.DIR_UPLOAD_SERVICES;
	...
	public function detail($slug){
		...
		$this->template->write('meta_image', PATH_URL . DIR_UPLOAD_SERVICES . $data["item"][0]->image);
