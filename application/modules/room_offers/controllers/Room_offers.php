...
class Room_offers extends MX_Controller {
	...
	public function admincp_update($id=0){
		...
		$data = array(
			'cates'=>$this->model->getDataAccommodations(),
			'result'=>$result[0],
			'module'=>$this->module,
			'id'=>$id
		);
	...
	public function admincp_save(){
		...
		if($_POST){
			...
			if($this->model->saveManagement($fileName)){
				...
				if($_FILES){
					if($_FILES){
						$upload_path = BASEFOLDER.DIR_UPLOAD_OFFERS;
	...
	public function admincp_restore(){
		...
		if($this->input->post('id')){
			...
			foreach ($result as $key => $value) {
				if($value->delete == 1){
					if(in_array($value->id, $listCates)){
	...
	public function detail($slug){
		...
		$this->template->write('meta_image', PATH_URL . DIR_UPLOAD_OFFERS . $data["item"][0]->image);
