		...
		<table class="table table-striped table-bordered table-hover dataTable no-footer">
			<thead>
				<tr role="row">
					...
					<th class="sorting" onclick="sort('title_vn')" id="title">Title_vn</th>
					<th class="sorting" id="condition">Condition(day)</th>
					<th class="sorting" id="title">Discount</th>
					<th class="sorting" onclick="sort('cate_id')" id="cate_id">Offers of rooms</th>
			...
			<tbody>
				...
				<tr class="item_row<?=$i?> gradeX <?php ($k%2==0) ? print 'odd' : print 'even' ?>" role="row">
					...
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><img src="<?=resizeImage(PATH_URL.DIR_UPLOAD_OFFERS.$v->image,150, 150)?>" /></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=$v->title_vn?></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=$v->condition?> days</a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=$v->discount?> %</a></td>
				...
				<?php $i++;}
					...
					<tr style="background:#c6c6c6;" class="item_row<?=$i?> gradeX" role="row">
						<td><input type="checkbox" id="item<?=$i?>" onclick="selectItem(<?=$i?>)" value="<?=$v->id?>"></td>
						<td class="center"><?=$k+1+$start?></td>
						<td><img src="<?=resizeImage(PATH_URL.DIR_UPLOAD_OFFERS.$v->image,150, 150)?>" /></td>
						<td><?=$v->title_vn?> </a></td>
						<td><?=$v->condition?> days</a></td>
						<td><?=$v->discount?> %</td>
						<td><?=$v->cate_name?></td>
