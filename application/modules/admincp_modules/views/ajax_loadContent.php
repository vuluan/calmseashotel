							...
							<table class="table table-striped table-bordered table-hover dataTable no-footer">
								<thead>
									<tr role="row">
										...
										<th class="sorting" onclick="sort('name')" id="name">Module</th>
										<th width="50">Order</th>
										<th class="center sorting" width="60" onclick="sort('status')" id="status">Action</th>
								...
								<tbody>
									...
									<tr class="item_row<?=$i?> gradeX <?php ($k%2==0) ? print 'odd' : print 'even' ?>" role="row">
										...
										<td class="center" id="loadStatusID_<?=$v->id?>"><a class="no_underline" href="javascript:void(0)" onclick="updateStatus(<?=$v->id?>,<?=$v->status?>,'<?=$module?>')"><?php ($v->status==0) ? print '<span class="label label-sm label-success">Install</span>' : print '<span class="label label-sm label-default">Uninstall</span>' ?></a></td>
